...
- [x] 2026-07-01 01:45 UTC Stage B eval queue 补跑完成：`runs/unitree_g1_23dof_eval_queue/stage_b_eval_queue_20260701T012312Z.log`，产出 39 个 ignored eval JSON，覆盖 13 checkpoints x 3 seeds。
...
- [ ] SI-4: Gate SI validation runner 生成 validation/safety/negative-control/memory-specificity reports；R018 未完成前只能写 paired matched-seed diagnostic。
- [ ] SI-5: strategy/runtime policy registry、rollback manifest 和 registry inspection CLI。
...
- [x] R007g: 在最多 3 张空闲 GPU 上跑 `Unitree-G1-23Dof-ForwardFlat` multi-seed。
...
- [ ] 如果目标是公司 G1 edu 23DoF，必须有 23DoF profile smoke；如果只跑 29DoF reference，验收中必须明确标注 reference-only。本轮为 `company_g1_edu_23dof` Stage B candidate，但 profile smoke 仍归 R007j，Viser play sanity 不能替代 smoke。
- [x] Morning Acceptance 必须列出每个 controller run 的 GPU、seed、task、run name、artifact path、OOM/失败情况和 gate decision。证据：`refine-logs/G1_23DOF_CONTROLLER_STAGE_B_ACCEPTANCE_20260701.md`。
- [x] 若固定直行仍明显斜走，停在 R007g/R007i 修 command distribution / reward，不进入 failure protocol。本轮 Viser 观察直线部分符合要求，转入 R007j smoke。
