...
这份时间线是“每天怎么推进”的执行说明，不是必须照天数硬跑的日历。真正的顺序由 gate 决定：当前 gate 没过，就停下来修当前问题，不进入下一阶段。
...
- [x] 锁定 Python、MuJoCo 和 MJLab/classic MuJoCo first backend policy；JAX/JAXLIB 仅作为 deferred Playground extra。controller checkpoint 和 robot XML/MJCF hash 仍是未选择阻塞字段，不能假装已经就绪。
