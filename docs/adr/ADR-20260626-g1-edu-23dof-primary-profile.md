...
- [x] 将当前 `mjlab_g1_29dof_reference` 保留为 reference smoke / upstream compatibility profile，而不是公司目标本体证据。
- [x] 记录 23DoF 官方 URDF/MJCF source、source commit、file hash 和 URDF joint order。
- [x] 在 23DoF project-local asset integration、actuator config、native controller 或 validated conversion experiment 和 adapter smoke 锁定前，不启动 controller-native baseline。
- [x] 如果短期找不到成熟 23DoF controller，允许保留 29DoF reference smoke 作为 backend health check，但所有文档必须写清楚它不是公司 G1 evidence path。
...
3. [x] 公司 23DoF MJCF/MJLab asset 是否已有官方或内部版本？官方 Unitree URDF/MJCF source 已定位，raw MuJoCo asset compile smoke 已通过；project-local MJLab wrapper 仍未完成。
4. [x] 是否存在 23DoF edu velocity controller checkpoint？当前未选择 mature 23DoF checkpoint；R007e 决策为 `train_23dof_required`，23DoF contract 为 action `23`、MJLab flat actor obs `81`、deploy-style obs `80`。见 `docs/g1_edu_23dof_controller_route.md`。
...
- [x] 修改 smoke scripts，使 expected action/obs dim 变成显式参数，并在维度不符时 fail。
- [x] 修改 Gate A tests，拒绝把 29DoF smoke 当作 23DoF evidence。
- [x] 完成 R007d raw asset compile smoke；证据：`docs/g1_edu_23dof_compile_smoke.md`。
- [x] 完成 R007e controller route/contract lock；证据：`docs/g1_edu_23dof_controller_route.md`、`src/humanoid_locomotion_runtime/controller_contracts.py`。
- [x] 同步 README、PRD、Gate A、backend lock、checkpoint selection、experiment plan、timeline、tracker 和 MANIFEST。
