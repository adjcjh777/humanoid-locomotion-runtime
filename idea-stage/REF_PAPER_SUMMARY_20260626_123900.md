...
这份文献报告是在找“最容易撞车的近邻工作”。读它时先抓住三点：

- RACER、VLM failure recovery、FLARE 等工作已经覆盖“语言/视觉帮助失败恢复”的大叙事，所以我们不能把 language recovery 当主卖点。
- HoRD、Chameleon 等工作威胁 “memory 有用” 这个说法，所以我们的 claim 必须收窄到 frozen locomotion controller 之上的 supervisory 层。
- HumanoidBench、MuJoCo Playground、LocoMuJoCo 是外部锚点；我们的增量必须落在 typed seeded failure protocol 和 memory-value diagnostic。

正式写论文前，所有 `verify_pending` 或由网页候选得到的信息都要重新核验 PDF、DOI 和 BibTeX。
