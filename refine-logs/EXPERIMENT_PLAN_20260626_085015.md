...
- [ ] R007e: company G1 23DoF MJLab wrapper/controller selection 完成；记录 23DoF action/obs shape 和 controller source，若无成熟 checkpoint 就明确 defer/train/convert。
...
|-----------|------|------|-----------|------|------|
| M0 | A800 single-host automation ready + robot profile lock | R000-R009a | DONE for M0 smoke/handoff；29DoF MJLab reference smoke passed；company G1 23DoF source identified and raw MuJoCo asset compile passed | low | 23DoF project-local MJLab wrapper, controller checkpoint and controller smoke are still pending |
