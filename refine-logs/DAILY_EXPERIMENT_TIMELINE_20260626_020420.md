...
- [x] 定位并锁定 G1 model/backend reference：`../mjlab`、`Mjlab-Velocity-Flat-Unitree-G1`、G1 MJCF 和 `VelocityOnPolicyRunner`；证据：`configs/environment.lock.toml`、`docs/mjlab_backend_lock.md`。
- [ ] 解决完整 MJLab dependency environment，并确认当前 runtime repo 能 import 选定 MJLab G1 task。
