...
- [x] R007: environment lock scaffold 完成；证据：`.python-version`、`pyproject.toml`、`uv.lock`、`configs/environment.lock.toml`、`docs/mjlab_backend_lock.md`；当前策略为 `../mjlab` Unitree G1 velocity backend first，JAX/JAXLIB 仅 deferred optional。
...
|-----------|------|------|-----------|------|------|
| M0 | A800 single-host automation ready | R000-R005 | DONE for M0 smoke/handoff；MJLab backend / G1 asset / wrapper reference selected on 2026-06-26 | low | controller checkpoint and full MJLab dependency smoke still pending |
...
- [x] Gate A repo foundation + environment lock completed；证据：`docs/gate_a_foundation.md`、`docs/mjlab_backend_lock.md`、`pyproject.toml`、`uv.lock`、`src/`、`tests/`、`.github/workflows/ci.yml`、`LICENSE`、`configs/environment.lock.toml`、`configs/artifact_retention.toml`；backend reference 已锁定为 `../mjlab` Unitree G1 velocity backend。
