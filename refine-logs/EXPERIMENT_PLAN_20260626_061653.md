...
- [x] R007: environment lock scaffold 完成；证据：`.python-version`、`pyproject.toml`、`uv.lock`、`configs/environment.lock.toml`、`docs/mjlab_backend_lock.md`、`docs/controller_checkpoint_selection.md`、`scripts/mjlab_sync_and_smoke.sh`；当前策略为项目内 `third_party/mjlab` Unitree G1 velocity backend first，完整 MJLab G1 headless simulation smoke 已通过，官方 Unitree ONNX controller artifact candidate 已下载到 ignored `checkpoints/` 并记录 hash，JAX/JAXLIB 仅 deferred optional。
...
|-----------|------|------|-----------|------|------|
| M0 | A800 single-host automation ready | R000-R005 | DONE for M0 smoke/handoff；project-local MJLab backend / G1 asset / wrapper reference selected on 2026-06-26；full MJLab G1 headless simulation smoke passed；official Unitree ONNX candidate downloaded | low | controller adapter smoke still pending because ONNX input `[1,98]` differs from MJLab actor obs `[1,99]` |
