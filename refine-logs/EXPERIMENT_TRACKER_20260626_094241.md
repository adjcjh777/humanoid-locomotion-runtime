...
| R018 | M1 | snapshot branching contract | snapshot/restore/common random numbers | unit/dev seeds | deterministic restore, branch metadata | MUST | 白天+夜间 | TODO | R018a metadata contract 已完成，但 deterministic restore/branch smoke 未完成；未通过前不得写 counterfactual / ATE / branch oracle |
| R018a | M1 | snapshot branch metadata contract | contract/testbed only | unit/dev seeds | branch metadata schema pass | MUST | 白天：人工 | DONE | 证据：`src/humanoid_locomotion_runtime/snapshot_branching.py`、`src/humanoid_locomotion_runtime/schemas.py`、`tests/test_snapshot_branching.py`、`docs/gate_c_option_snapshot_contract.md`；contract-only，不表示 R018 restore 已通过 |
