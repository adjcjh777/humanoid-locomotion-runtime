...
| R007 | M0 | environment lock scaffold | pyproject/uv/version pins | n/a | lock inputs listed | MUST | 白天：人工 | DONE | 证据：`.python-version`、`pyproject.toml`、`uv.lock`、`configs/environment.lock.toml`、`docs/mjlab_backend_lock.md`、`docs/controller_checkpoint_selection.md`、`scripts/mjlab_sync_and_smoke.sh`；Python/MuJoCo/MJLab-first policy 已 pin，项目内 `third_party/mjlab` Unitree G1 backend、MJCF 和 wrapper hashes 已锁定；完整 MJLab G1 headless simulation smoke 已通过；官方 Unitree ONNX controller artifact candidate 已放入 ignored `checkpoints/` 并记录 hash，但 input `[1,98]` 与 MJLab actor obs `[1,99]` 需要 adapter；JAX/JAXLIB 移到 deferred Playground extra |
