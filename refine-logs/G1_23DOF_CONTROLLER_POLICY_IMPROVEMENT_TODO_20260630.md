...
- [x] Stage B post-training eval queue 已启动并重建：当前 watcher `g1vb_pack_eval_after_train_20260630T085653Z` 会等待 packed runs 的 `model_10000.pt` 和训练 tmux 结束后，再用 GPU `5` 分批评估 Stage B checkpoints。
- [ ] Stage B / VelocityBalancedFlat multi-seed 完成：至少 3 个 seed 到 `Learning iteration 10000/10001`，目标是通用 velocity controller，不牺牲固定直行。
- [ ] 对每个 run 的 `model_*.pt` 做 command-grid eval，记录 lateral drift、yaw error、velocity error、fall/done fraction。
- [x] 比较 `model_10000.pt` 与新训练更后期 checkpoint；截至 Stage A 结束，当前 best checkpoint 仍是 `model_5000.pt`，最终轮不应默认导出为 best。
...
- [ ] 至少一个 23DoF policy candidate 通过 command-grid eval，固定直行不明显斜走。
- [ ] ONNX/export contract 仍为 `obs [1,80] -> actions [1,23]`。
...
- [ ] `refine-logs/EXPERIMENT_TRACKER.md`、`EXPERIMENT_PLAN.md`、`DAILY_EXPERIMENT_TIMELINE.md`、`MANIFEST.md` 同步 gate decision。
