...
- [x] R007: environment lock scaffold 完成；证据：`.python-version`、`pyproject.toml`、`uv.lock`、`configs/environment.lock.toml`、`docs/mjlab_backend_lock.md`、`docs/controller_checkpoint_selection.md`；当前策略为项目内 `third_party/mjlab` Unitree G1 velocity backend first，官方 Unitree ONNX controller artifact candidate 已下载到 ignored `checkpoints/` 并记录 hash，JAX/JAXLIB 仅 deferred optional。
...
|-----------|------|------|-----------|------|------|
| M0 | A800 single-host automation ready | R000-R005 | DONE for M0 smoke/handoff；project-local MJLab backend / G1 asset / wrapper reference selected on 2026-06-26；official Unitree ONNX candidate downloaded | low | controller adapter smoke and full MJLab dependency smoke still pending |
...
- [x] Gate A repo foundation + environment lock completed；证据：`docs/gate_a_foundation.md`、`docs/mjlab_backend_lock.md`、`docs/controller_checkpoint_selection.md`、`pyproject.toml`、`uv.lock`、`src/`、`tests/`、`.github/workflows/ci.yml`、`LICENSE`、`configs/environment.lock.toml`、`configs/artifact_retention.toml`；backend reference 已锁定为项目内 `third_party/mjlab` Unitree G1 velocity backend，controller artifact candidate 已锁定来源和 hash。
