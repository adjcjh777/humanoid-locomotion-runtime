...
- environment lock：Python、MuJoCo、JAX/JAXLIB、CUDA wheel、controller checkpoint、robot XML/MJCF 版本与 hash；
- core schemas；
...
- 若 controller smoke gate 仍不能稳定执行 `stand_ready`、`track_velocity`、`safe_stop`、short `walk_to`，则主证据路径切到 MuJoCo Playground。
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| mature G1 controller 难以集成 | 阻塞 runtime | 设置 controller smoke gate；失败则切 MuJoCo Playground 证据路径 |
